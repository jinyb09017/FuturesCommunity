...
$(function(){
	$('#txtPassword').blur(function check_pwd(){
		...
		if(pwd==''){
			$('#password\\.info').html('请输入密码！');
			 checkFlag.pwd=false;
			 $('#pwdInfoimage').attr("src","images/unchecked.gif");
		}else{
			var reg=/^[A-Za-z0-9]{6,20}$/;
			if(reg.test(pwd)){
				$('#password\\.info').html('密码输入正确！');
				 checkFlag.pwd=true;
				 $('#pwdInfoimage').attr("src","images/checked.gif");
			}else{
	...
	$('#txtRepeatPass').blur(function check_Repeatpwd(){
		...
		if(repeatpwd==''){
			...
			$('#password1Image').attr("src","images/unchecked.gif");
		}else{
			var reg=/^[A-Za-z0-9]{6,20}$/;
			if(reg.test(repeatpwd)){
				if(pwd==repeatpwd){
					$('#password1\\.info').html('密码一致！');
					 ...
					 $('#password1Image').attr("src","images/checked.gif");
				}else{
	...
	$('#txtNickName').blur(function check_nickname(){
		...
		if(nickname==''){
			...
			checkFlag.nickname=false;
			 $('#nicknameImage').attr("src","images/unchecked.gif");
		}else{
			var reg=/^[\u4E00-\u9FA5\w\d]{4,20}$/;
			if(reg.test(nickname)){
				$('#name\\.info').html('昵称可用！');
				checkFlag.nickname=true;
				 $('#nicknameImage').attr("src","images/checked.gif");
			}else{
	...
	$('#regEmail').blur(function(){
		...
		if(email==''){
			...
			checkFlag.email=false;
			$('#emailImage').attr("src","images/unchecked.gif");
			return ;
		}
		...
		$.post("user/validEmail.action",{"email":email},function(ok){
			if(ok){
				$('#email\\.info').html('邮箱可用');
				checkFlag.emailajax=true;
				$('#emailImage').attr("src","images/checked.gif");
			}else{
	...
	$('#txtVerifyCode').blur(function (){
		if($('#txtVerifyCode').val()==""){
		 ...
		 $('#numberImage').attr("src","images/unchecked.gif");
		 return ;
		}
		//alert('1');
		$.post("user/validate.action",{"code":$("#txtVerifyCode").val()},function(data){
			//alert("data.ok :"+ data.ok+" data:"+data.code);
			
		if(data.ok){
		//alert('2');
		$('#number\\.info').html('验证码正确');
		checkFlag.code=true;
		 $('#numberImage').attr("src","images/checked.gif");
		}else{
		//alert('3');
		...
		$(this).attr("src","user/imageCode.action?dt="+new Date().getTime());
		});
